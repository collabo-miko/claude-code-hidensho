...
> `cp` は `Copy-Item` に読み替えてください。
> フックスクリプトには `.ps1` 版を用意しています（例: `scan-secrets.ps1`）。
...
> **重要: `jq` のインストールが必要です**
>
> このサンプルのフックは JSON パーサー `jq` を使用しています。
> `jq` が未インストールだとフックが無視され、**ブロックが機能しません。**
>
> **macOS:**
> ```bash
> brew install jq
...
> ```
> インストール後、**ターミナルを開き直してください**（PATH の反映に必要）。
>
> **確認:**
> ```
> jq --version
...
- **前提条件:** フック内で `jq` を使用しています。未インストールだとフックが無視され、ブロックが機能しません（事前準備を参照）
