...
> `.sh` スクリプトには `.ps1` 版を用意しています（例: `bash setup-demo.sh` → `.\setup-demo.ps1`）。

> **注意:** `/cost` コマンドは API 直接利用（Max プラン等）の場合のみ使用可能です。
> Teams プランではコンテキスト消費の確認に `/context` を使用してください。

> **前提条件:** Hooks 演習（演習 8〜11）のフックスクリプトは `jq` を使用しています。
> 未インストールだとフックが無視されます。インストール方法は [04_hooks の事前準備](../04_hooks/README.md#事前準備) を参照してください。
