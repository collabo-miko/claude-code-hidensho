...
/model opus           ← 複雑な設計時のみ
/model opusplan       ← Plan は Opus、実行は Sonnet で自動切替
```
...
**Step 1:** `Ctrl+C` で抜けて、ターミナル:
```bash
bash setup-demo.sh    # デモ用ビルド成果物を生成
claude
```
...
セッション内:
```
/context              ← コンテキスト消費の差を確認
```

> **重要:** 確認後、deny ルールを元に戻すのを忘れずに！
...
セッション内:
```
/context              ← メインのコンテキストにテストコード + 全出力が入っている
```
